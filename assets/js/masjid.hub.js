...
function getJamaahAvailableFeatures() {
  ...
  const pkg = (selectedMosque.package || 'basic').toLowerCase();
...
// ── MOSQUE DETAIL MODAL ──────────────────────────────────────────
function showMosqueDetail(mosqueId) {
  const mosque = MOSQUES.find(m => m.id === mosqueId);
  if (!mosque) {
    showNotif('Data masjid tidak ditemukan.', 'error');
    return;
  }

  const detailModal = document.getElementById('mosque-detail-modal');
  if (!detailModal) return;

  document.getElementById('detail-mosque-name').textContent = mosque.name;
  document.getElementById('detail-mosque-icon').textContent = mosque.icon || '🕌';
  document.getElementById('detail-mosque-address').textContent = mosque.address;
  document.getElementById('detail-mosque-city').textContent = mosque.city;
  document.getElementById('detail-mosque-package').textContent = `Paket ${(mosque.package || 'basic').toUpperCase()}`;
  ...
  // Facilities
  const facilitiesMap = {
    1: ['Kamar Tamu', 'WiFi Gratis', 'Lounge Jamaah', 'Parkir Prioritas', 'AC', 'Kamar Mandi Bersama'],
    7: ['Ruang Serbaguna', 'Penginapan', 'Area Diskusi', 'AC', 'WiFi', 'Kantin'],
  };
  const facilities = facilitiesMap[mosqueId] || ['WiFi', 'AC', 'Toilet'];
  document.getElementById('detail-mosque-facilities').innerHTML = facilities.map(f =>
  ...
  const eventsMap = {
    1: [
      { name: 'Kajian Tafsir Al-Quran', time: 'Kamis, 19:00 WIB', location: 'Aula Utama' },
      { name: 'Bakti Sosial Jumat Berkah', time: 'Jumat, 10:30 WIB', location: 'Halaman Masjid' },
      { name: 'Pengajian Ahad Pagi', time: 'Ahad, 07:00 WIB', location: 'Aula Utama' },
      { name: 'Kelas Tahfidz Anak', time: 'Sabtu, 09:00 WIB', location: 'Ruang TPA' },
    ],
    7: [
      { name: 'Open House Penginapan', time: 'Minggu, 08:30 WIB', location: 'Area Penginapan' },
      { name: 'Seminar Kemuhaddizan', time: 'Sabtu, 14:00 WIB', location: 'Ruang Serbaguna' },
      { name: 'Kelas Bahasa Arab', time: 'Rabu, 16:00 WIB', location: 'Ruang Belajar' },
      { name: 'Pengajian Remaja', time: 'Jumat, 15:00 WIB', location: 'Aula Atas' },
    ],
  };
  const events = eventsMap[mosqueId] || [];
  document.getElementById('detail-mosque-events').innerHTML = events.map(e => `
    <div class="event-row">
      <div class="event-info">
        <p class="event-name">${e.name}</p>
        <p class="event-meta">${e.time} • ${e.location}</p>
      </div>
      <button onclick="registerEvent(${mosqueId},'${e.name}')" class="event-register-btn">Daftar</button>
    </div>
  `).join('') || '<p class="text-xs text-gray-500">Belum ada agenda kegiatan.</p>';

  // Accommodation
  const stayMap = {
    1: [
      { name: 'Kamar Tamu Standar', price: 'GRATIS', desc: 'Kasur, kipas angin, shared bathroom. Infaq suka rela.' },
      { name: 'Kamar Tamu Deluxe', price: 'GRATIS', desc: 'AC, kamar mandi dalam, TV. Infaq suka rela.' },
    ],
  };
  const stays = stayMap[mosqueId] || [];
  const staysContainer = document.getElementById('detail-mosque-stays');
  if (staysContainer) {
    if (stays.length > 0) {
      staysContainer.innerHTML = stays.map(s => `
        <div class="stay-package">
          <div class="stay-info">
            <p class="stay-name">${s.name}</p>
            <p class="text-xs text-gray-500">${s.desc}</p>
            <p class="stay-price-free">${s.price}</p>
          </div>
          <button onclick="bookAccommodation(${mosqueId},'${s.name}')" class="stay-book-btn">Pesan</button>
        </div>
      `).join('');
    } else {
      staysContainer.innerHTML = '<p class="text-xs text-gray-500">Tidak tersedia paket penginapan di masjid ini.</p>';
    }
  }
...
function closeMosqueDetail() {
  const modal = document.getElementById('mosque-detail-modal');
  if (modal) modal.classList.add('hidden');
}

function registerEvent(mosqueId, eventName) {
  if (!isLoggedIn) {
    showNotif('Silakan login terlebih dahulu untuk mendaftar kegiatan.', 'info');
    openLoginPortal();
    return;
  }
  showLoading('Mendaftar kegiatan...');
  setTimeout(() => {
    hideLoading();
    showNotif(`Berhasil mendaftar kegiatan "${eventName}"! Anda akan mendapat notifikasi jadwal.`, 'success');
  }, 1200);
}

function bookAccommodation(mosqueId, roomName) {
  const mosque = MOSQUES.find(m => m.id === mosqueId);
  const priceMap = {
    'standar': { nurul: 'Rp 75.000', zayed: 'Rp 100.000' },
    'deluxe': { nurul: 'Rp 150.000', zayed: 'Rp 200.000' },
    'family': { nurul: 'Rp 200.000', zayed: 'Rp 250.000' },
  };

  showLoading('Memproses pemesanan...');
  setTimeout(() => {
    hideLoading();
    const price = mosqueId === 1 ? priceMap[roomName]?.nurul : priceMap[roomName]?.zayed || roomName;
    showNotif(`Pemesanan "${roomName}" di ${mosque?.name || 'masjid'} berhasil! Harga: ${price}/malam. Konfirmasi akan dikirim via WhatsApp.`, 'success');
  }, 1200);
}
...
const SHOWCASE_MOSQUE_IDS = [1, 7];

function getMosqueById(id) {
  return MOSQUES.find(m => Number(m.id) === Number(id));
}

function renderMosqueShowcase() {
  SHOWCASE_MOSQUE_IDS.forEach(id => {
    const container = document.getElementById(`showcase-events-${id}`);
    if (!container) return;
    const events = MOSQUE_EVENT_CATALOG[id] || [];
    container.innerHTML = events.length
      ? events.map((ev, i) => `
          <div class="showcase-event-row">
            <div class="showcase-event-info">
              <p class="font-semibold text-sm">${escapeHtml(ev.title)}</p>
              <p class="text-xs text-slate-500 mt-1">${escapeHtml(ev.time)} · ${escapeHtml(ev.type)}</p>
            </div>
            <button onclick="joinMosqueEvent(${id}, ${i})" class="btn-outline btn-small">Join</button>
          </div>
        `).join('')
      : '<p class="text-xs text-slate-500">Belum ada kegiatan untuk masjid ini.</p>';
  });
}

function joinMosqueEvent(mosqueId, eventIndex) {
  const mosque = getMosqueById(mosqueId);
  const eventData = (MOSQUE_EVENT_CATALOG[mosqueId] || [])[eventIndex];
  if (!mosque || !eventData) return;
  showNotif(`Anda akan bergabung ke "${eventData.title}" di ${mosque.name}.`, 'success');
}

function requestStay(mosqueId) {
  const mosque = getMosqueById(mosqueId);
  if (!mosque) return;
  showNotif(`Permintaan menginap di ${mosque.name} telah dicatat. Silakan hubungi pengurus untuk konfirmasi.`, 'success');
}

function renderAnnouncements() {
  const announcementSection = document.getElementById('page-announcement');
  if (!announcementSection) return;
  // Konten statis sudah tersedia di HTML, tetapi fungsi ini dapat digunakan untuk menambah dynamic later.
}
...
function initApp() {
  ...
  // Mosque indicator badge
  const mosqueIndicatorName = document.getElementById('mosque-indicator-name');
  if (mosqueIndicatorName) mosqueIndicatorName.textContent = selectedMosque.name;
  // User
...
function showLockedFeatureModal(page) {
  ...
  const iconEl = document.getElementById('locked-modal-icon');
  ...
  const isHighestPackage = currentIndex === order.length - 1;

  // Update icon and title based on package status
  if (iconEl) {
    iconEl.className = isHighestPackage
      ? 'w-12 h-12 rounded-full bg-emerald-100 flex items-center justify-center text-xl'
      : 'w-12 h-12 rounded-full bg-amber-100 flex items-center justify-center text-xl';
    iconEl.textContent = isHighestPackage ? '✓' : '🔒';
  }
  if (titleEl) {
    titleEl.textContent = isHighestPackage ? 'Fitur Tidak Tersedia' : 'Fitur Terkunci';
  }
  if (msgEl) {
    if (isHighestPackage) {
      msgEl.textContent = `Paket tertinggi sudah aktif. Fitur premium utama sudah tersedia.`;
    } else {
      msgEl.textContent = `Fitur ini terkunci di paket ${currentPkg.toUpperCase()}. Pilih upgrade untuk membukanya.`;
    }
  }
  
  if (detailsEl) {
    if (isHighestPackage) {
      detailsEl.innerHTML = '<p class="text-emerald-600 text-center py-2">✓ Paket tertinggi sudah aktif. Semua fitur utama sudah tersedia.</p>';
    } else {
  ...
  if (upgradeActions) {
    ...
    } else if (isHighestPackage) {
      upgradeActions.innerHTML = '<p class="text-xs text-emerald-600 text-center font-semibold">✓ Paket tertinggi sudah aktif.</p>';
...
function navigate(page) {
  ...
  if (selectedMosque && activeRole !== 'takmir') {
    const pkg = (selectedMosque.package || 'basic').toLowerCase();
    const featureKey = PAGE_FEATURE_MAP[page] || page;

    // Premium mosques have all features unlocked
    if (pkg === 'premium') {
      // All features available, continue navigation
    } else if (pkg === 'standard') {
      // Standard: kajian is available, others need premium
      const standardFeatures = ['dashboard', 'profile', 'jadwal-sholat', 'pengumuman', 'kajian'];
      if (!standardFeatures.includes(featureKey)) {
        showLockedFeatureModal(page);
        return;
      }
    } else {
      // Basic: only basic features available
      const basicFeatures = ['dashboard', 'profile', 'jadwal-sholat', 'pengumuman'];
  ...
  // Mobile top tab bar
  document.querySelectorAll('.mobile-tab-btn').forEach(n => {
    n.classList.toggle('active', n.getAttribute('data-tab') === page);
  });
  // Page title
  const titles = { dashboard:'Dashboard', donasi:'Donasi', announcement:'Info & Pengumuman', zcorner:'UMKM Masjid', kajian:'Kajian & Acara', relawan:'Relawan', sholat:'Waktu Sholat', streak:'Streak Kebaikan', laporan:'Laporan Keuangan', riwayat:'Riwayat Donasi', booking:'Booking Ruangan' };
  ...
  // Update mosque indicator badge
  if (selectedMosque) {
    const badgeName = document.getElementById('mosque-indicator-name');
    if (badgeName) badgeName.textContent = selectedMosque.name;
  }
